...
| `packages/web-miner` | React + Vite miner on `packages/ui`: embedded wallet (IndexedDB), sponsored FPC, W proved by bb.js in a Worker, pinned CRS (`crs.lock.json`, served from `/crs`), Vitest specs, Playwright E2E on the isolated network; ships at `/mine/` of the assembled site (`wrangler.jsonc` for local `pages dev` parity only) |
...
bun run e2e:agent -- bun run site:e2e   # the assembled site under wrangler pages dev: paths, headers, build.json, the demo
