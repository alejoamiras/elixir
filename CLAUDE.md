...
| `packages/deploy` | `src/deploy.ts` (a profile's miner + token → `deployments/<profile>.json`; `YACANA_PORTAL` the portal it trusts, `YACANA_CONTINUE_FROM` a continuation), `scripts/l1-deploy.ts` (YACA + the portal through Foundry, the record's `bridge` block; `--anvil` for CI), `scripts/bridge.ts` + `src/bridge/` (the operator script: `status`, `register`, `note-transitions`, `pause`/`pause-all`/`unpause`, `close-deposits`, `set-forwarder`, `retire`, `forward` with the witness archive; `run.ts` deploys the bridge for one local run), `launch.ts`, `soak.ts`, `epoch-stats.ts`, the spike drivers |
...
YACANA_L1_PRIVATE_KEY=… [YACANA_RECORD=…] bun run bridge -- status|register|note-transitions|pause|pause-all|unpause|close-deposits|set-forwarder|retire|forward   # the operator script (docs/upgrades.md)
