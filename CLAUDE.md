...
| `protocol/work-circuit` | Noir work circuit `W` (`crates/lib` + `crates/yacana_work`), the VK-embedding verifier `crates/verify_w`, generated VK / proof-layout manifest, fixture proofs, `scripts/` (`prove.ts`, the `check:*` evidence scripts on `bb-verify.ts`'s three-way verdict, `export-vk.ts`, `layout-manifest.ts` (also the miner's point offsets, `proof_points.nr`), the pinned `toolchain.ts`) |
| `protocol/contracts` | Aztec contracts (Nargo workspace): `yacana_miner` (its `src/proof_points.nr` generated from the proof layout) and `yacana_bridge_hashes`; aztec-standards token as a git dep (`v5.2.0`) |
