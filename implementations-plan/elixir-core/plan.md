...
**Status**: v6 — at the approval gate.
...
```
packages/
  contracts/        Noir · Nargo workspace: elixir_miner (ours) + aztec-standards token ported to aztec-nr 5.2.0
  ...
  web-miner/        React + Vite + Tailwind v4 + shadcn · Worker-hosted bb.js backend · @aztec/wallet-sdk
...
Reuse per `recon.md`: aztec-standards token (ported), aztec-nr state vars, TXE, bb.js backends; nothing from the prototype as code.
...
- Flow: connect wallet (`@aztec/wallet-sdk`) → read open epoch → generate the epoch secret → mine → on win, wallet proves `claim` (Chonk) → balance refresh; epoch switch mid-proof discards the in-flight nonce and rotates the secret.
...
- **Fee economics**: a claim is a Chonk tx with a ~50–60k-gate app circuit; its fee is paid in the fee asset, and a brand-new miner holds none. Phase 1 reports gas/resource usage, fee-asset cost and the break-even ELX price at which `REWARD` covers it; the mainnet fee path is Ask 10.
...
- **Mainnet**: `/harden security` **required** before mainnet (Ask 8). No pause: parameter mistakes are fixed by redeploying (Ask 6).
...
20. The 5.2.0 reference token has an admin-settable `minters` map (`noir-contracts/contracts/app/token_contract/src/main.nr:53,141`); aztec-standards' single immutable minter is the reason to port it.
...
- I5. The aztec-standards token source ports to aztec-nr 5.2.0 with a small, auditable delta. Time-boxed: if the delta is large, fork the 5.2.0 reference token and replace its `minters` map with a `PublicImmutable<AztecAddress>` (~20 lines).
...
1. **Parameters**: `N` = **24** claims per epoch; `EXPECTED_EPOCH_SECONDS` = **3600**; `T_MAX` = **4 × 3600**; `REWARD` = **4 ELX** → ≈96 ELX/hour at steady state, perpetual; decimals **18**. Initial target calibrated so one M-series laptop closes epoch 0 in about an hour (≈ 2^122 for ~1000 proofs/hour). The simulator reports the expected stale-ticket rate for the chosen `N` (~8 % at 24).
2. **Work size**: `CHAIN_LEN` calibrated for **≈3 s per proof in WASM on M-series** (Phase 1 output; native is ~10× faster).
3. **Aztec version**: **5.2.0** everywhere; aztec-standards token **ported** (git dep at a 5.2-compatible commit if one exists, else vendored with the delta reviewed). Never downgrade the repo.
4. **Go/no-go for Phase 1** — GO requires all of: a valid claim, sent through a `@aztec/wallet-sdk` wallet running the real prover, is accepted by a sandbox node; tampered proofs fail to prove through the recursive path for **every transcript phase** of the layout manifest (native: all 410 fields); **ECCVM rows for the full claim tx ≤ 2^14** (Fact 16); repeated W proofs byte-identical; **early-abort cost ≥ 90 %** of full `generateProof` cost at the point the digest becomes computable; **the known disabled-row attack implemented and measured** (Fact 17), costing ≥ **50 %** of an honest prove — a measured attack cost, without implying cheaper attacks are excluded; in-browser claim p95 ≤ **2 min**, peak memory ≤ **3 GB**, no worker OOM, on the reference laptop; **claim fee reported** as gas/resource usage, fee-asset cost and break-even ELX price (no threshold: dimensionally it cannot be compared to `REWARD` without a price); VK pinning confirmed in ACIR (I10); bootstrap dry run passes (I12). Anything else is **NO-GO → return to the user** with Appendices B then A.
5. **Miner secret**: **per-epoch random secret** generated locally, kept for the epoch, discarded at close (Fact 18 rules out a wallet-signature KDF; tickets die with their epoch so nothing long-lived needs protecting). Confirm.
6. **Immutability**: miner and token immutable, `auth_contract = 0`; parameter mistakes → redeploy + documented migration (new token, no automatic conversion). Confirm.
7. **Testnet**: current public Aztec testnet at Phase 5; sponsored FPC.
8. **Mainnet gate**: `/harden security` **required**, plus an **independent external audit** of the work circuit, VK, contract and recursion path — the user decides budget and timing; the plan blocks mainnet on both.
9. **Tier**: `mid` against a `deep` recommendation — acknowledged.
10. **Fee path** — needs your explicit approval: testnet = sponsored FPC. Mainnet: a new miner holds no fee asset. Default: **miners bring fee juice (bridged from L1), documented in the miner README**; alternative: a later plan ships an FPC that accepts ELX. Phase 1 reports the claim's gas/resource usage and fee-asset cost, and the break-even ELX price at which `REWARD` covers it (no ELX/fee-asset price is assumed by the plan).
11. **Bootstrap authority**: the deployer key that calls `bind_token` once (§3.4). Default: **the same key that deploys, used once, then irrelevant**; no multisig for a one-shot call. Confirm.
12. **Genesis premine**: the deployer can pre-mine epoch 0 (bounded to `N × REWARD` = 96 ELX at defaults by count-capping). Default: **accepted and disclosed**, no fair-launch ceremony. Confirm.
...
### Phase 4 — Web miner
App per §3.6; Vitest (status panel, difficulty formatting, worker reducer, epoch-switch mid-proof and secret rotation); Playwright E2E on the isolated sandbox with a **`@aztec/wallet-sdk` wallet running the real prover** (not a PXE shortcut): connect, mine at easy target, claim, balance shown; `crossOriginIsolated` asserted; worker crash → recovery; peak memory recorded; malformed **and lying** RPC payloads (wrong epoch params) rejected by schema and cross-check. `wrangler.jsonc` + `_headers` (COOP/COEP/CSP); deploy manual.
**Gate**: `bun run test:components && bun run --cwd packages/web-miner build && bun run e2e:agent -- web-miner` exit 0; lint. Layers: unit, e2e-live.

### Phase 5 — Testnet soak + docs + mainnet readiness
Testnet deploy via `packages/deploy` (sponsored FPC); addresses and parameters in `docs/deployments.md`; epoch-stats script; **soak: ≥ 24 closed epochs (≥ 24 h wall-clock at the default cadence — this gate blocks the PR for that long, by design) with hashrate deliberately varied (stop/start miners, add a second machine) so the retarget data is non-trivial**; miner README incl. the fee path (Ask 10); threat-model doc from §4 with the measured ticket-cost figures (attack costs, not lower bounds); `docs/roadmap.md` with aztec-accelerator integration and the mainnet checklist (`/harden security` + external audit, Ask 8).
**Gate**: deploy script exit 0 with receipts; soak report committed; `bun run lint && bun test` green; docs updated in the same PR. Layers: lint, unit, live (testnet).
...
1. **`/code-review <level> --fix`** on the arc's diff, level from §7 (A1 low, A2 medium, A3 medium; never `max`). Skim the applied fixes for unintended changes; **commit them separately** from implementation commits.
