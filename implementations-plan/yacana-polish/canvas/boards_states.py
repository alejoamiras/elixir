...
DEADLINE_SET = "until Mar 17 (the upgrade after V6 is already scheduled; later only if the bridge pauses)"
...
K1 = [
    ...
    ("ready, after the 180 days, the upgrade after V6 not seen yet", "could close any day", "bad", "The 180 days are over. V5's exits close the day the upgrade after V6 lands. Claim it now.", "Claim on Ethereum"),
...
K2 = [
    ...
    ("headroom", "waiting for the limit", "warn", "More has left V5 than its exit limit allows right now. Yacana forwards it once there is room, or you do from V6; a redeem waits the same way.", ""),
    ...
    ("held, after the 180 days, the upgrade after V6 not seen yet", "could close any day", "bad", "The 180 days are over. V5's exits close the day the upgrade after V6 lands. Forward it from V6 or redeem it now.", "Forward to V6 · Redeem on Ethereum"),
...
def ahead_rows():
    ...
    r3b = activity("3.5 tYACA", AHEAD, "The 180 days are over. V5's exits close the day the upgrade after V6 lands. Forward it from V6 or redeem it now.",
...
def wallet_reasons():
    ...
    t4 = bal("The bridge is paused until Sep 20: claims on Ethereum wait until it lifts. Sending and deposits still work; a send now waits on Ethereum.", set())
    body = ('<div class="board" style="padding:24px"><span class="lm">the balance tile · when a button is off, it says why; a pause only warns</span>'
