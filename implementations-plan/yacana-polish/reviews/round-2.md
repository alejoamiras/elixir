...
| 4 | high | "In order; nothing to do" invents a queue: `_requireOpen` checks headroom at execution. | Confirmed. | **accept**. "More has left V5 than its exit limit allows right now. The limit grows by the hour while V5 is current, and this turns ready to claim once it fits; others may use the room first." (K2: "Yacana forwards it once there is room, or you do from V6; a redeem waits the same way."). |
...
the phone keeping the full summary) are all in v3.
