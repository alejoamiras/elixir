...
```
bun run --cwd apps/web-stats dev        # generates the slot table and layouts, then Vite
bun run --cwd apps/web-stats build      # the production bundle (dist/, with _headers)
```

## Tests

```
bun run --cwd apps/web-stats test:components                     # Vitest: charts, the strip, the URL selection
bun run e2e:agent -- bun run --cwd apps/web-stats test:e2e       # Playwright on an isolated network: the captured
                                                                     # history through a mocked node, a deep link, the live deployment + Verify
```

## The screenshot gate

`bun run --cwd apps/web-stats test:visual` renders the observatory on the captured history (`e2e/visual-rpc.json`
...
Update the baselines in a reviewed PR: `bun run --cwd apps/web-stats test:visual --update-snapshots`. Re-record
the answers after a change to what the page reads: `bun run e2e:agent -- bun apps/web-stats/e2e/visual-setup.ts
record`.
