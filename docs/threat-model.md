...
| Roll-seed pre-mining | the escape-hatch seed is public from `opened_at + T_MAX`; a sequencer-miner choosing the slot gains at most the claims it proves in its head start, still ≤ `N` | bounded in the simulator |
