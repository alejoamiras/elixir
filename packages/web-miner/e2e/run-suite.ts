// `bun run test:e2e [playwright args…]`: prebuild, Playwright, the breakdown, each on its own clock;
// exits with Playwright's code. E2E_SHARD=<name> runs shards.json's files under that name and
// requires their inventory executed: Playwright is silent when a shard matches nothing, and a
// skipped test leaves its exit code green.
import { spawnSync } from 'node:child_process';
import { readFileSync, rmSync } from 'node:fs';
import { resolve } from 'node:path';
import { titlesOf } from './proof-inventory.ts';
import { BREAKDOWN_FILE, coverageGap, executedTitles, REPORT_FILE, reportRun } from './report.ts';
...
for (const f of [REPORT_FILE, TIMINGS_FILE, BREAKDOWN_FILE]) rmSync(f, { force: true });
