...
describe('the lifecycle block', () => {
  test('a stop is noted once, on the version’s own record, as a unix time; the node after the stop', () => {
    ...
    expect(lifecycleAfter(v5, 'note-stop', '5', '1700000000')).toEqual({ stoppedProvingAt: '1700000000' });
    expect(() => lifecycleAfter(v5, 'note-stop', '6', '1700000000')).toThrow(/the record is version 5/);
    expect(() => lifecycleAfter(v5, 'note-stop', '5', '2026-09-21')).toThrow(/not a unix time/);
    expect(() => lifecycleAfter(v5, 'note-stop', '5', '17000000000')).toThrow(/not a unix time/);
    expect(() => lifecycleAfter(v5, 'note-stop', '5', '1700000001', 1700000000)).toThrow(/in the future/);
    const stopped = { ...v5, lifecycle: { stoppedProvingAt: '1700000000' } };
    expect(() => lifecycleAfter(stopped, 'note-stop', '5', '1700000001')).toThrow(
      /already noted at 1700000000/,
    );
    expect(() => lifecycleAfter(v5, 'retire-node', '5', '')).toThrow(/note-stop first/);
    expect(lifecycleAfter(stopped, 'retire-node', '5', '')).toEqual({
      stoppedProvingAt: '1700000000',
      nodeRetired: true,
    });
    expect(() =>
      lifecycleAfter(
  ...
  test('the write lands in the record beside everything else, and only there', () => {
    ...
    expect(writeLifecycle(path, 'note-stop', '5', '1700000000')).toEqual({ stoppedProvingAt: '1700000000' });
    ...
    expect(JSON.parse(readFileSync(file, 'utf8'))).toEqual({
      ...
      rollupVersion: '5',
      bridge: { portal: '0x1' },
      lifecycle: { stoppedProvingAt: '1700000000', nodeRetired: true },
    });
  });
});
