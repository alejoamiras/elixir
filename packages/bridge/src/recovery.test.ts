...
describe('the recovery file', () => {
  ...
  test('a malformed entry names itself: a bad state, a bad id, a bad witness, a foreign version', () => {
    ...
    const twin = { ...crossing, id: `${crossing.id}:6:99`, inboxIndex: '99', target: '6' } as Crossing;
    expect(
      parseRecoveryFile(JSON.stringify(recoveryFile(scope, [twin])), { chainId: '31337', portal: PORTAL })
        .crossings[0]?.id,
    ).toBe(`${crossing.id}:6:99`);
    ...
    expect(parse(file({ proofDeadline: 'not-a-date' }))).toThrow(/proofDeadline is not a whole number/);
    expect(parse(file({ proofDeadline: '9000000000000' }))).toThrow(/proofDeadline is not a time/);
    expect(parse(file({ inboxIndex: '1e3' }))).toThrow(/inboxIndex is not a whole number/);
